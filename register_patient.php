...
<div class="container-fluid">
	...
	<div class="contentheader">
			<i class="icon-table"></i> Patient Registration on: <?php echo date("l jS F Y"); ?>
			</div>
			<ul class="breadcrumb">
...
<a  href="dashboard.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			...
			include('connect.php');
				$result = $db->prepare("SELECT * FROM patients ORDER BY id DESC");
				$result->execute();
			...
			Total Number of patient registered:  <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $rowcount;?>]</font>
			</div>
</div>
...
<a rel="facebox" href="addpatient.php?staff=<?php echo $staff ?>"><Button type="submit" class="btn btn-info" style="float:right; width:230px; height:35px;" /><i class="icon-plus-sign icon-large"></i> Add Patient</button></a><br><br> 
			...
			<?php
				...
				for($i=1; $row = $result->fetch(); $i++){
                    ...
                    <tr class="record" style="text-transform:lowercase;">
                        ...
                        <td><?php echo $row['demographic']; ?></td>
                       
                        <td><a  title="Click To print " rel="facebox" href="print_patient_detail.php?id=<?php echo $row['id']; ?>&name=<?php echo $row['name']?>&address=<?php echo $row['address']; ?>&hospital_no=<?php echo $row['hospital_no']; ?>&yschima_no=<?php echo $row['yschima_no']; ?>
						    &sex=<?php echo $row['sex']; ?>&nhis_no=<?php echo $row['nhis_no']; ?>&phone_no=<?php echo $row['phone_no']; ?>&demographic=<?php echo $row['demographic']; ?>&age=<?php echo $row['age']; ?>">
						<button class="btn btn-primary btn-mini"><i class="icon-print"></i> print </button></a> 
                        <a  title="Click To Edit " rel="facebox" href="edit.php?id=<?php echo $row['id']; ?>"><button class="btn btn-warning btn-mini"><i class="icon-edit"></i> Edit </button></a> 
