
<div class="span2" style="margin-top:2%;">
             <div class="well sidebar-nav" style="padding: 9px 0;">
                 <ul class="nav nav-list">
              <li class="active"><a href="dashboard.php"><i class="icon-dashboard icon-2x"></i> Dashboard </a></li> 
			
			<li><a href="register_patient.php"><i class="icon-list-alt icon-2x"></i> register patient</a>                                     </li>

			<li><a href="record.php?d1=0&d2=0"><i class="icon-bar-chart icon-2x"></i> Record</a>                </li>
			<li><a href="admin-settings.php"><i class="icon-table icon-2x"></i> User Manager</a>                </li>
			<!--<li><a href="amount.php"><i class="icon-money icon-2x"></i> Amount</a>                </li>-->
				<br><br><br><br><br><br><br><br><br><br><br>	
			<li>
			 <div class="hero-unit-clock">
		
			<form name="clock">
			<font color="white">Time: <br></font>&nbsp;<input style="width:150px;" type="submit" class="trans" name="face" value="">
			</form>
			  </div>
			</li>
				
				</ul>           
          </div><!--/.well -->
        </div><!--/span-->