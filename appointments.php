...
<div class="container-fluid">
	...
	<div class="contentheader">
			<i class="icon-table"></i> Patient Appointment on: <?php echo date("l jS F Y"); ?>
			</div>
			<ul class="breadcrumb">
...
<a  href="dashboard.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			<?php 
			include('connect.php');
				$result = $db->prepare("SELECT * FROM appointment ORDER BY appointment_id DESC");
				$result->execute();
			...
			Total Number of active appointment:  <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $rowcount;?>]</font>
			</div>
</div>
			...
			<?php
				...
				for($i=1; $row = $result->fetch(); $i++){
                    ...
                    <tr class="record" style="text-transform:lowercase;">
                        ...
                        <td>
                        <a  title="Click To Edit " rel="facebox" href="edit.php?id=<?php echo "a"//$row['patient_id']; ?>"><button class="btn btn-warning btn-mini"><i class="icon-edit"></i> Edit </button></a> 
			            <a href="#" id="<?php echo "a"//$row['id']; ?>" class="delbutton" title="Click To Delete"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Delete</button></a></td>
                    </tr>
                <?php
                    }
                ?>
